...
class EbsHelper(object):
    ...
    def wait_for_environments(self, environment_names, health=None, status=None, version_label=None,
                              include_deleted=True, wait_time_in_seconds=600):
        """
        ...
        # print some stuff
        s = "Waiting {} seconds for environment(s) ".format(wait_time_in_seconds) + (
        ...
        while True:
            ...
            if elapsed > wait_time_in_seconds:
                raise Exception("Wait time for environment(s) " + (
                " and ".join(environment_names)) + " to be " + health + " expired")
